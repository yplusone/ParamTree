...
sys.path.extend(["../", "./"])
from database_util.knob_utils import construct_dividing_line
import numpy as np
...
class Knob:
    def __init__(self, name, **kwargs):
        ...
        if self.type == 'bool':
            ...
            if kwargs.get('max')>1000000:
                self._max = min(kwargs.get('max'), float(kwargs.get('default'))* 4)
                if self.type == 'int':
    ...
    @property
    def userset(self):
        return self.to_numeric(self.user_set)
...
def load_knobs_from_json_file():
    filename = './data/util/conf.json'
